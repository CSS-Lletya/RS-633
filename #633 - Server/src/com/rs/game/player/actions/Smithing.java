...
public class Smithing extends Action {
	...
	int amount = ForgingInterface.getForgedAmount(bar.getItems()[index].getId());
	if(bar.getItems()[index].getDefinitions().isStackable())
	    amount *= Settings.CRAFT_RATE;
	player.getInventory().addItem(bar.getItems()[index].getId(), amount);
