...
public class PickPocketAction extends Action {
	...
	private boolean isSuccesfull(Player player) {
		/*
		 * int thievingLevel = player.getSkills().getLevel(Skills.THIEVING); int
		 * increasedChance = getIncreasedChance(player); int level =
		 * Utils.getRandom(thievingLevel + increasedChance); double ratio =
		 * level / npcData.getThievingLevels()[0]; if (Math.round(ratio *
		 * thievingLevel) < npcData.getThievingLevels()[0] /
		 * player.getAuraManager().getThievingAccurayMultiplier()) return false;
		 */
