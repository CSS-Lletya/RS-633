...
public class JewllerySmithing {
	...
	for (int primaryIndex = 0; primaryIndex < MOLDS.length; primaryIndex++) {
		...
		for (int secondaryIndex = 0; secondaryIndex < ITEMS[primaryIndex].length; secondaryIndex++) {
		    player.getPackets().sendItems((299 + (primaryIndex * 14) + secondaryIndex), new Item[] { new Item(!player.getInventory().containsItemToolBelt(GEMS[secondaryIndex]) ? NULL_ITEMS[primaryIndex] : ITEMS[primaryIndex][secondaryIndex]) });
		}
	    } else
