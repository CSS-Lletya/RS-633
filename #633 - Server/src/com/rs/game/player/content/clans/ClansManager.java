...
public class ClansManager {
	...
	public static void viewClanmateDetails(Player player, ClanMember member) {
		...
		player.getPackets().sendGlobalConfig(1565,
				member.isBanFromKeep() ? 1 : 0);
		player.getPackets().sendGlobalConfig(1566,
				member.isBanFromCitadel() ? 1 : 0);
		player.getPackets().sendGlobalConfig(1567,
				member.isBanFromIsland() ? 1 : 0);
		player.getPackets().sendGlobalConfig(1568, member.firstWeek() ? 1 : 0);
		player.getPackets().sendGlobalString(347,
	...
	public static void unlockBanList(Player player) {
		player.getPackets()
				.sendUnlockIComponentOptionSlots(1110, 11, 0, 100, 0);
	}
	...
	public static void openClanMottifInterface(Player player) {
		...
		for (int i = 0; i < manager.clan.getMottifColors().length; i++)
			player.getVarsManager().sendVar(2094 + i,
	...
	public void setMottifColor(Player player, int color, int part) {
		synchronized (this) {
			player.getVarsManager().sendVar(2094 + part,
	...
	public void generateClanSettingsDataBlock() {
		...
		if (version >= 3) {
			...
			if (clan.getThreadId() != null)
				count++;
			if (clan.isRecruiting() || clan.isClanTime()
					|| clan.getWorldId() != 0 || clan.getClanFlag() != 0)
				count++;
			...
			if (clan.isRecruiting() || clan.isClanTime()
				...
				stream.writeInt((clan.isRecruiting() ? 1 : 0)
						| (clan.isClanTime() ? 1 : 0) << 1
						| clan.getWorldId() << 2 | clan.getClanFlag() << 10);
			}
			if (clan.getMottifTop() != 0 || clan.getMottifBottom() != 0) {
				stream.writeInt(13 | 0 << 30);
				stream.writeInt(clan.getMottifTop()
						| clan.getMottifBottom() << 16);
			}
			stream.writeInt(16 | 0 << 30);
			stream.writeInt(clan.getMottifColors()[0]
					| clan.getMottifColors()[1] << 16);
			stream.writeInt(18 | 0 << 30);
			stream.writeInt(clan.getMottifColors()[2]
	...
	public static void openClanSettings(Player player) {
		...
		player.getPackets().sendIComponentText(1096, 373,
				"Permissions are currently disabled and setted to default.");
		...
		// permissions
		// setting
		player.getPackets().sendUnlockIComponentOptionSlots(1096, 240, 0, 144,
				0); // unlocks
		// timezone
		// setting
		player.getPackets().sendUnlockIComponentOptionSlots(1096, 290, 0, 200,
				0); // unlocks
		// worldid
		// setting
		player.getPackets().sendUnlockIComponentOptionSlots(1096, 41, 0,
				Clan.MAX_MEMBERS, 0); // unlocks
		// clanmates
		player.getPackets().sendUnlockIComponentOptionSlots(1096, 276, 0, 125,
				0); // set
		// member
		// rank
		player.getPackets().sendUnlockIComponentOptionSlots(1096, 262, 0, 500,
				0); // set
		// member
		// profession
	}
