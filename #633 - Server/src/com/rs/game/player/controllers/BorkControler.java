...
public class BorkControler extends Controller {
	...
	if (borkStage == 0) {
	    player.getInterfaceManager().setWindowInterface(player.getInterfaceManager().hasRezizableScreen() ? 1 : 11, 692);
	} else if (borkStage == 1) {
	    for (Entity t : bork.getPossibleTargets()) {
		Player pl = (Player) t;
		pl.getInterfaceManager().setWindowInterface(pl.getInterfaceManager().hasRezizableScreen() ? 1 : 11, 691);
	    }
	}
    }
